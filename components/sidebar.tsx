...
export function Sidebar() {
  ...
  const activeSection = useActiveSection(["home", "about", "experience", "projects", "tech", "cv", "contact"])
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-[100svh] lg:h-screen w-64 bg-sidebar/80 backdrop-blur-xl border-r border-border/60 shadow-2xl shadow-black/30 transition-all duration-500 z-40 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          }`}
      >
          ...
          <div className="mb-10 pt-4 animate-fade-in space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">Estudante de IA · Dados</p>
                <p className="text-xs text-muted-foreground">Transição para a área de Dados</p>
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  className="p-2 rounded-lg border border-border/60 hover:border-primary/50 hover:bg-primary/10 transition-all duration-300 touch-manipulation"
            ...
            <nav className="space-y-6">
              {navItems.map((item, idx) => (
                ...
                >
                  {item.label}
                  <span
                    className={`absolute left-0 top-1/2 -translate-y-1/2 h-1.5 w-1.5 rounded-full bg-gradient-to-r from-primary via-secondary to-accent transition-all duration-300 ${activeSection === item.id
                        ? "opacity-100 scale-100 shadow-[0_0_18px_rgba(120,245,255,0.65)]"
                        : "opacity-0 scale-50 group-hover:opacity-60 group-hover:scale-100"
                      }`}
                  />
                  <span className="absolute left-0 bottom-0 h-0.5 w-0 bg-gradient-to-r from-primary via-secondary to-accent group-hover:w-full transition-all duration-300" />
                </a>
              ))}
            </nav>
