...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (frameCountRef.current % 240 === 0) {
        wordIndexRef.current = (wordIndexRef.current + 1) % activeWords.length
        nextWord(activeWords[wordIndexRef.current])
      }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-black p-4">
      <canvas
        ref={canvasRef}
        aria-label="Animated particle text canvas"
